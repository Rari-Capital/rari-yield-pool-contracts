...
contract("RariFundController, RariFundManager", accounts => {
  ...
  it("should upgrade the fund controller owner", async () => {
    ...
    // Test disabling and enabling the fund from the new owner address
    await fundControllerInstance.setFundDisabled(true, { from: process.env.DEVELOPMENT_ADDRESS_SECONDARY });
    await fundControllerInstance.setFundDisabled(false, { from: process.env.DEVELOPMENT_ADDRESS_SECONDARY });
  ...
  it("should disable and re-enable the fund", async () => {
    ...
    await fundControllerInstance.setFundDisabled(false, { from: process.env.DEVELOPMENT_ADDRESS, nonce: await web3.eth.getTransactionCount(process.env.DEVELOPMENT_ADDRESS) });
