...
contract("RariFundManager, RariFundController", accounts => {
  it("should deposit to the fund, approve and deposit to pools, accrue interest, and withdraw from the fund", async () => {
    ...
    for (const poolName of Object.keys(pools)) for (const currencyCode of Object.keys(pools[poolName].currencies)) {
      ...
      let postDepositPoolBalance = await fundControllerInstance.getPoolBalance.call(["dYdX", "Compound", "Aave", "mStable", "yVault"].indexOf(poolName), currencyCode);
      ...
      var acceptMarginOfError = (poolName == "dYdX" && currencyCode != "DAI") || poolName == "mStable" || (poolName == "yVault" && ["DAI", "TUSD"].indexOf(currencyCode) < 0);
      ...
      let preWithdrawalPoolBalance = await fundControllerInstance.getPoolBalance.call(["dYdX", "Compound", "Aave", "mStable", "yVault"].indexOf(poolName), currencyCode);
